...
RULES = [
    ...
    (r"(congratulations|you\s+have\s+won|lucky\s+winner)", 0.8, "Lottery/Prize Scam"),
    (r"(lottery|jackpot|prize|winnings)", 0.7, "Lottery Mention"),
    (r"(send|provide|share).*(account\s+details|bank\s+details|credentials)", 0.9, "Bank Details Request"),
    (r"(claim|collect).*(prize|lottery|winnings)", 0.6, "Prize Claim Instruction"),
...
def detect_intent(text: str):
    matches = []
    total_score = 0.0
    max_possible = sum(weight for _, weight, _ in RULES)
